...
const compile = () => Promise.resolve()
  ...
  .then((stats) => {
    ...
    logger.info(`Copying static assets from ./pixi-live2d to ./${project.outDir}.`)
    fs.copySync(
      path.resolve(project.basePath, project.live2dPath),
      path.resolve(project.basePath, project.outDir, project.live2dPath)
    )
    return stats
  })
  .then((stats) => {
